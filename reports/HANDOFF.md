...
| `expense-tracker-v23` | 2026-09-19 — | `4f341a9` | Run 35428836779, `workflow_dispatch`, success; both jobs green. Carries `BEL-01` and `CLB-01` — the bell learning the repayment schedule, and the debt row gaining the act that finishes the debt. Verified both ways: the live `index.html` is byte-identical to the repository's (`md5 2fe868c817b1c9943fe298a56ed8d5bc`) and carries `data-debt-reminder-settle`, `Mark settled</button>`, `debtNextAgreedPayment` and `Agreed payment:`, none of which existed in v22. Live `sw.js` reads v23. **Eighth consecutive bump, on the same reading:** v22 was live and v22 was staged, checked by fetching the deployed `sw.js` before touching the file. **Third deploy of the day, and the first carrying a change to a surface outside the Debts module** — the notification sheet is shared by Planned, Goals and Debts, and three of its four branches were examined to change one. |
